# Evidence Map

## Legend

- **Strong**: multiple human RCTs/meta-analyses, clinically meaningful outcomes
- **Moderate**: some human trial evidence or strong observational evidence
- **Early**: plausible but limited human evidence
- **Speculative**: mostly animal/mechanistic, commercial hype, or unclear endpoints

## Intervention categories

### Exercise and function

- Resistance training: **Strong; newer RCT meta-analysis evidence supports resistance exercise as the clearest intervention for improving muscle-specific strength in adults ≥60, with nutrition and aerobic training still important but not substitutes for progressive loading**
- Aerobic training / VO₂ max: **Strong**
- Reducing prolonged uninterrupted sitting: **Moderate observational evidence; a 2026 UK Biobank wearable cohort of 89,010 participants linked ≥10 h/day sedentary time plus low sedentary-break frequency with higher all-cause and cardiovascular mortality and fewer CVD-free years, supporting practical movement-break discussions while preserving causality and falls-risk cautions**
- Balance and fall prevention: **Strong for improving balance/physical-function measures; a large 2026 Bayesian network meta-analysis of 423 RCTs in adults ≥60 supports exercise for balance and adds modality-specific nuance, while a 2026 frailty-focused meta-analysis shows short-term gait/TUG/sit-to-stand gains but limited direct fall-incidence evidence in physically frail adults**
- Power training for older adults: **Moderate; a 2026 RCT meta-analysis suggests eccentric-overload training can improve lower-limb strength and explosive power in older adults, but small trials, equipment needs, soreness/tendon risk, and heterogeneous protocols make clinician/exercise-professional supervision important**
- Multidomain frailty interventions combining home/community exercise with nutrition, cognitive, psychological, social, environmental, and daily-function supports: **Moderate; a 2026 network meta-analysis of 22 RCTs in community-dwelling prefrail/frail older adults ranked exercise+cognitive approaches highly for frailty reduction and exercise+nutrition+psychological approaches highly for motor ability, while preserving heterogeneity/ranking uncertainty. Large harmonized cohort evidence also suggests that frailty trajectory can dominate BMI category for stroke-risk stratification, reinforcing functional assessment alongside cardiometabolic assessment**
- Multidomain lifestyle intervention for cognitive-risk older adults: **Moderate; LatAm-FINGERS and SUPERBRAIN-MEET support structured exercise/diet/cognitive-social/vascular-risk programs for cognitive composite outcomes in at-risk older adults, but dementia-incidence, independence, and implementation evidence remain developing**

### Nutrition and body composition

- Adequate protein intake: **Moderate to Strong**
- Mediterranean-style dietary pattern: **Strong**
- Creatine for older adults: **Moderate**
- Aggressive calorie restriction or intentional weight loss in older adults: **Risk-dependent; caution; LOSE-AF suggests supervised weight loss in older adults with overweight and persistent AF can be feasible without trial-related serious safety concerns, but it did not improve AF symptoms/burden over 8 months and lean-mass/frailty risks still need individualized monitoring**

### Cardiometabolic risk

- Blood pressure control: **Strong; exercise is a core non-pharmacological adjunct, and a 2026 RCT network/dose-response meta-analysis in adults ≥45 found BP reductions across modalities, with combined/circuit aerobic-resistance approaches ranking favorably but not definitively superior**
- ApoB/LDL-C lowering when indicated: **Strong**
- Diabetes prevention/management: **Strong; long-term DPP/DPPOS follow-up supports intensive lifestyle intervention for lower multimorbidity burden in adults with prediabetes, whereas metformin did not show a comparable multimorbidity association in that analysis**
- SGLT2 inhibitors: **Strong for indicated type 2 diabetes/heart failure/chronic kidney disease contexts; older-adult randomized-subgroup meta-analysis in non-dialysis CKD supports renal and heart-failure benefit while reinforcing monitoring for genital infections and volume depletion/hypotension; emerging observational data suggest no excess geriatric-syndrome signal in older adults with type 2 diabetes, newer absolute-benefit models may help clinician-led targeting for primary heart-failure prevention in type 2 diabetes, and DapaTAVI extends disease-specific evidence to selected elderly patients after TAVI for severe aortic stenosis**
- GLP-1 receptor agonists: **Strong for indicated obesity/diabetes contexts, with stronger disease-specific cardiorenal evidence in CKD/type 2 diabetes/ASCVD-risk contexts; monitor muscle/frailty implications and evolving safety signals including rare ocular events, mixed musculoskeletal/fall-fracture data, and unresolved cancer-signal surveillance. A 2026 body-composition meta-analysis suggests adiposity reduction is plausible but muscle quantity/quality evidence remains low-certainty, and FLOW infection analyses are encouraging only for high-risk T2D+CKD contexts, so older-adult monitoring should include indication strength, nutrition, strength, physical function, hydration, and routine cancer screening rather than body weight alone**

### Medication safety and polypharmacy

- Structured medication review / deprescribing conversations: **Strong as a safety and function-preservation practice; a 2026 BMJ population-level cohort of 2.3 million adults ≥66 prioritized 24 potentially inappropriate prescribing cascades, supporting clinician/pharmacist review of whether new symptoms may be medication adverse effects before adding another drug**

### Geroscience compounds

- Rapamycin/rapalogs: **Early for longevity; stronger in specific disease contexts**
- Metformin for longevity in non-diabetics: **Early/uncertain**
- NAD precursors / NAD-boosting formulations: **Early; biomarker effects do not prove outcome benefit; a short phase 0/1b LNAD+ trial showed rapid intracellular NAD increases over 5 days without clinical endpoint improvement, and a 2026 Itabashi older-adult cross-sectional study linked lower whole-blood NAD+ with frailty but found attenuation after hematocrit adjustment, reinforcing biomarker-only caution**
- Senolytics: **Early/speculative outside trials**

### Diagnostics and tracking

- Standard labs and risk-factor tracking: **Strong**
- Handgrip strength and simple physical-performance measures as risk markers: **Moderate to Strong for risk stratification; not treatment-directing by themselves**
- Intrinsic-capacity / multidomain geriatric assessment concepts: **Moderate for risk stratification; implementation should be clinician-guided; newer Health and Retirement Study evidence suggests dynamic deterioration or recovery in intrinsic capacity tracks later cognitive decline and dementia risk, and newer BMI-frailty trajectory data suggest functional frailty meaningfully modifies long-term stroke risk**
- Continuous glucose monitoring (CGM): **Strong for selected insulin-treated type 2 diabetes contexts; not a general longevity tracker**
- DEXA for bone/body composition: **Moderate to Strong depending on indication; post-fall hospitalization and fragility-fracture cohorts reinforce clinician-led bone-health evaluation after serious falls or low-trauma fractures, including in older men. A 2026 BMJ network meta-analysis of 124 randomized exercise trials supports weight-bearing aerobic activity and combined aerobic-resistance exercise for BMD preservation at roughly 600 MET-min/week, while weighted-vest exercise has smaller RCT meta-analysis support versus usual activity; exercise remains an adjunct rather than a substitute for safe resistance/balance training or indicated osteoporosis care**
- Cognitive resilience and Alzheimer’s biomarker context: **Moderate for risk framing, not self-directed treatment; a 2026 Nature Medicine longitudinal cohort found that biomarker pathology and cognitive resilience independently and jointly predicted incident Alzheimer’s dementia, reinforcing attention to modifiable resilience domains while preserving caution that resilience scores and blood biomarkers are not stand-alone treatment-directing tools**
- Epigenetic clocks: **Early; not yet treatment-directing; recent cohort evidence suggests conventional risk factors can outperform epigenetic clocks for chronic-disease prediction, muscle-aging associations remain modest/sex-specific, smoking-cessation duration tracks more favorable methylation-aging profiles in NHANES, and 2026 reliability work shows biological instability can limit interpretation even when technical reproducibility is high**
- Polygenic longevity scores: **Early for personal decision-making; a Finnish cohort found modest mortality association, but smoking behavior and conventional risk factors were more practically important predictors and scores are not treatment-directing**
- Consumer biological age panels: **Use cautiously**

### Infection resilience

- Age-appropriate vaccination (for example influenza, COVID-19, RSV, pneumococcal, shingles per current clinical guidance): **Strong for preventing infection-related morbidity in indicated older adults; a 2026 US Medicare cohort with median age 74 found RSV vaccination effectiveness of 81.8% against RSV-related hospitalization and 85.0% against RSV-related death in the 2023-2024 season, adding to real-world adjuvanted RSVPreF3 effectiveness evidence against hospitalization/complications in adults ≥60. 2025/26 Stockholm data support seasonal influenza vaccine protection against hospitalization in adults ≥65, and I-MOVE hospitalized-case data link influenza vaccination with lower in-hospital death among adults ≥65; observational vaccine-effectiveness estimates should still be interpreted cautiously because healthy-vaccinee/frailty-related selection effects can be large**
- RSV vaccination and cardiopulmonary resilience: **Moderate to Strong product/season-specific observational evidence; a large US claims study found adjuvanted RSVPreF3 vaccination associated with fewer RSV-related MACE and severe COPD/asthma exacerbation events in adults ≥60 with baseline disease, but residual confounding and current guidance still matter**
- Combined influenza/COVID-19 mRNA vaccines: **Promising/investigational product-specific evidence for durable immunogenicity; require authorization and outcome/surveillance data before treatment-directing use**
- Post-COVID cardiovascular and long-COVID vigilance: **Moderate observational evidence for an early post-infection cardiovascular risk window, especially in older adults; newer target-trial emulation evidence does not show a meaningful third-dose effect on long-COVID incidence, so booster decisions should remain anchored to current severe-disease prevention guidance**
